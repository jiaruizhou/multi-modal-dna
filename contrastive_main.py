...
from contrastive_finetune import train_one_epoch, evaluate
...
def main(args):
    ...
    fuse_model = contrastive_dna_model(args)
    ...
    if args.eval:
        test_stats = evaluate(args=args, data_loader=data_loader_val, model=fuse_model, criterion=criterion, device=device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(fuse_model, criterion, data_loader_train, optimizer, device, epoch, loss_scaler, args.clip_grad, log_writer=log_writer, args=args)
